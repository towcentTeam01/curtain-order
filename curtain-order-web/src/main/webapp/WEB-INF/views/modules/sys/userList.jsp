...
<body>
	...
	<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/user/list" method="post" class="navbar-form form-search" role="form">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}" />
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}" />
		<input id="userType" name="userType" type="hidden" value="${user.userType}" />
		...
		<ul class="ul-form">
			<li>
				...
				<sys:treeselect id="company"
					...
					allowClear="false" sysSelect="true" />
			</li>
			<li >
			...
			<li class="btns">
				<button id="btnSubmit" type="submit" class="button big" onclick="return page();">查询</button>
			<%--	<button id="btnExport" type="button" class="button big">导出</button>
				<button id="btnImport" type="button" class="button big">导入</button> --%>
			</li>
			<li class="clearfix"></li>
		</ul>
		
<%-- 		<ul class="ul-form">
			<li><label>归属公司：</label> <sys:treeselect id="company"
					name="company.id" value="${user.company.id}"
					labelName="company.name" labelValue="${user.company.name}"
			...
			<li><label>角色名：</label> <form:input path="roleName"
					htmlEscape="false" maxlength="50" class="input-medium" /></li>
			<li class="clearfix"></li>
